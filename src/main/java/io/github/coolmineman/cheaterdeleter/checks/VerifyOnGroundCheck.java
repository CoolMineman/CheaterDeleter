    ...
    public ActionResult onMovementPacket(CDPlayer player, PlayerMoveC2SPacketView packet) {
        if (packet.isChangePosition()) {
            if (packet.isOnGround()) {
                Box playerBox = CollisionUtil.getBoxForPosition(player, packet.getX(), packet.getY(), packet.getZ()).expand(0.1);
                // Not having a smaller feetbox shouldn't matter unless head is in a block lol
                assertOrFlag(!CollisionUtil.isTouching(playerBox, player.getWorld(), CollisionUtil.touchingTopPredicate(playerBox)), player, FlagSeverity.MAJOR, "Spoofed onGround true");
            } else {
                Box playerBox = CollisionUtil.getBoxForPosition(player, packet.getX(), packet.getY(), packet.getZ()).expand(-0.1);
                Box feetBox = new Box(playerBox.minX, playerBox.minY, playerBox.minZ, playerBox.minX, playerBox.maxY, playerBox.minZ);
                assertOrFlag(!CollisionUtil.isTouching(feetBox, player.getWorld(), CollisionUtil.touchingTopPredicate(feetBox)), player, FlagSeverity.MAJOR, "Spoofed onGround false");
            }
        } else if (packet.isOnGround() && !player.isOnGround()) {
            Box playerBox = CollisionUtil.getBoxForPosition(player, player.getX(), player.getY(), player.getZ()).expand(0.4); //Should be liberal enough
            // Not having a smaller feetbox shouldn't matter unless head is in a block lol
